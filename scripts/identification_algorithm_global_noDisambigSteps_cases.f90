...
!Kevin Lupo
!klupo@ucar.edu
!identify cutoff lows and preexisting troughs using TroughCutoff algorithm
!
!29 Sep 2021
...
!30 Sep 2021
!	Compute for NH and SH, omitting poles and tropics. rmax in TroughCutoff does not require cross-pole gridpoint matching
...
!	For cleaner tracking, keep only features with So > 5.0 m/100km. This is consistent with the climatology of TroughCutoff
!11 Nov 2021
...
!
!15 Sep 2023
!	Modified this version of the code to run on UFS-MRW case study output
!	Removed temporary addition of Munoz wind reversal output. Code still performs the calculation, but it is excluded from I/O
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
use netcdf
use netcdf_routines_mod, only : get_netcdf_dims, open_netcdf, close_netcdf, get_netcdf_var_1d_real
implicit none
...
integer				:: nx, ny, nr, nt, dims(4),ndims
...
call get_command_argument(2,itime)
call get_command_argument(3,fhour)

call get_command_argument(4,maxr_c)
read(maxr_c, *) maxr
call get_command_argument(5,minr_c)
read(minr_c, *) minr
call get_command_argument(6,numr_c)
read(numr_c, *) nr
call get_command_argument(7,srmax_c)
read(srmax_c, *) srmax
call get_command_argument(8,cyclic)
call get_command_argument(9,debug)
call get_command_argument(10,outdir)
call get_command_argument(11,smooth)
call get_command_argument(12,somin_c)
read(somin_c, *) somin
...
call nc_check( nf90_open(trim(infile), nf90_nowrite, ncIDi), "TroughCutoff", "open " // trim(infile) )

call get_netcdf_dims(ncIDi,"XLONG",dims,ndims,.false.)
nx = dims(1)
ny = dims(2)
nt = dims(3)
...
call nc_check( nf90_inq_varid(ncIDi, "XLAT", var_id), "TroughCutoff", "inq_dimid latitude" )	    
call nc_check( nf90_get_var(ncIDi, var_id, lat2d), "TroughCutoff", "get_var lat" )
call nc_check( nf90_inq_varid(ncIDi, "XLONG", var_id), "TroughCutoff", "inq_dimid longitude" )
call nc_check( nf90_get_var(ncIDi, var_id, lon2d), "TroughCutoff", "get_var lon" )


call nc_check( nf90_inq_dimid(ncIDi, "Time", var_id), "TroughCutoff", "inq_dimid Time" )
call nc_check( nf90_get_var(ncIDi, var_id, time), "TroughCutoff", "get_var Time" )
lat = lat2d(1,:)
lon = lon2d(:,1)
do x=1,nx		    
  if(lon(x).lt.0)then
...
!!!!!! Former fix for coordinate array problems. Seems fixed by using different netcdf commands. 
!!!!!! Leaving commented code below in case it is necessary in future uses.
!!!!!! Gridded latitudes are from 0-180, rather than 90- -90. Fix here
!print *,lat
!write(*,*),lon
!stop
!lat = lat-90.0
...
!!!!!!Read model data
call nc_check( nf90_inq_varid(ncIDi, "z850", var_id), "TroughCutoff", "inq_varid z850" )
call nc_check( nf90_get_var(ncIDi, var_id, z850), "TroughCutoff", "get_var z850" )

call nc_check( nf90_inq_varid(ncIDi, "z500", var_id), "TroughCutoff", "inq_varid z850" )
call nc_check( nf90_get_var(ncIDi, var_id, hgt), "TroughCutoff", "get_var z850" )

call nc_check( nf90_inq_varid(ncIDi, "z200", var_id), "TroughCutoff", "inq_varid z200" )
call nc_check( nf90_get_var(ncIDi, var_id, z200), "TroughCutoff", "get_var z200" ) 

call nc_check( nf90_inq_varid(ncIDi, "t850", var_id), "TroughCutoff", "inq_varid t850" )
call nc_check( nf90_get_var(ncIDi, var_id, t850), "TroughCutoff", "get_var t850" )

call nc_check( nf90_inq_varid(ncIDi, "t500", var_id), "TroughCutoff", "inq_varid t500" )
call nc_check( nf90_get_var(ncIDi, var_id, t500), "TroughCutoff", "get_var t500" )

call nc_check( nf90_inq_varid(ncIDi, "t200", var_id), "TroughCutoff", "inq_varid t200" )
call nc_check( nf90_get_var(ncIDi, var_id, t200), "TroughCutoff", "get_var t200" )

call nc_check( nf90_inq_varid(ncIDi, "u850", var_id), "TroughCutoff", "inq_varid u850" )
call nc_check( nf90_get_var(ncIDi, var_id, u850), "TroughCutoff", "get_var u850" )

call nc_check( nf90_inq_varid(ncIDi, "u500", var_id), "TroughCutoff", "inq_varid u500" )
call nc_check( nf90_get_var(ncIDi, var_id, u500), "TroughCutoff", "get_var u500" )

call nc_check( nf90_inq_varid(ncIDi, "u200", var_id), "TroughCutoff", "inq_varid u200" )
call nc_check( nf90_get_var(ncIDi, var_id, u200), "TroughCutoff", "get_var u200" ) 

call nc_check( nf90_inq_varid(ncIDi, "v850", var_id), "TroughCutoff", "inq_varid v850" )
call nc_check( nf90_get_var(ncIDi, var_id, v850), "TroughCutoff", "get_var v850" )

call nc_check( nf90_inq_varid(ncIDi, "v500", var_id), "TroughCutoff", "inq_varid v500" )
call nc_check( nf90_get_var(ncIDi, var_id, v500), "TroughCutoff", "get_var v500" )

call nc_check( nf90_inq_varid(ncIDi, "v200", var_id), "TroughCutoff", "inq_varid v200" )
call nc_check( nf90_get_var(ncIDi, var_id, v200), "TroughCutoff", "get_var v200" )

call nc_check( nf90_inq_varid(ncIDi, "rh850", var_id), "TroughCutoff", "inq_varid rh850" )
call nc_check( nf90_get_var(ncIDi, var_id, rh850), "TroughCutoff", "get_var rh850" )

call nc_check( nf90_inq_varid(ncIDi, "rh500", var_id), "TroughCutoff", "inq_varid rh500" )
call nc_check( nf90_get_var(ncIDi, var_id, rh500), "TroughCutoff", "get_var rh500" )

call nc_check( nf90_inq_varid(ncIDi, "rh200", var_id), "TroughCutoff", "inq_varid rh200" )
call nc_check( nf90_get_var(ncIDi, var_id, rh200), "TroughCutoff", "get_var rh200" )

!!!!!!Close the netcdf file
call nc_check( nf90_close(ncIDi), "TroughCutoff", "close " // trim(infile) )
call cpu_time(FN)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!! Smooth the height field using a 9-point smoother !!!!!!!!

call cpu_time(ST)
hgt_raw = hgt						! Save the unsmoothed field for area-averaged values to be passed to txt output file
call smth9(hgt,nx,ny,SMP,SMQ,NSM)			! smth9(var , num x, num y, wgt for "up/down/left/right", wgt for "diagonals", num passes)
call cpu_time(FN)
...
!!!!Write to text file!!!!
  open(1, file=trim(outfile), status="unknown")

  fstr = "(A7, 1x, A7, 1x, A11, 1x, A7, 1x, A7, 1x, A7, 1x, A4, 1x, A15, 1x, A16, 1x, A16, 1x, A7, 1x, A7, 1x, A7, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A17, 1x, A17, 1x, A17, 1x, A17, 1x, A17)"
  write(1,fstr) "ITIME","FHOUR","So(m/100km)","LAT(N)","LON(E)","Ro(km)","SR","BGo(m/100km)","BGo-lat(m/100km)","BGo-lon(m/100km)","ZMIN(m)","ZLAT(N)","ZLON(E)","Z850(m)","Z500(m)","Z200(m)","T850(K)","T500(K)","T200(K)","U850(m/s)","U500(m/s)","U200(m/s)","V850(m/s)","V500(m/s)","V200(m/s)","MR850(g/kg)","MR500(g/kg)","MR200(g/kg)","600kmZ500(m)","600kmT500(K)","600kmU500(m/s)","600kmV500(m/s)","600kmMR500(g/kg)"

  fstr = "(A11, 1x, A4, 1x, F6.2, 1x, F6.2, 1x, F6.2, 1x, F10.2, 1x, F6.2, 1x, F6.2, 1x, F6.2, 1x, F6.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F5.1,1x,F5.1,1x,F5.1,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.4,1x,F7.4,1x,F7.4,1x,F8.2,1x,F5.1,1x,F7.2,1x,F7.2,1x,F7.4)"
  do b=1,np  
    write(1,fstr) itime, fhour, features(b,1), features(b,2), features(b,3), features(b,4)/1000.0, features(b,5), features(b,6), features(b,7), features(b,8), features(b,9), features(b,10), features(b,11), features(b,12), features(b,13), features(b,14), features(b,15), features(b,16), features(b,17), features(b,18), features(b,19), features(b,20), features(b,21), features(b,22), features(b,23), features(b,24), features(b,25), features(b,26), features(b,27), features(b,28), features(b,29), features(b,30), features(b,31)
  end do 
  close(1)
  call cpu_time(FN)
...
! Create the output file
    ...
    call nc_check( nf90_create( trim(doutfile), nf90_clobber, ncIDo ), "TroughCutoff", "open TroughCutoff_slope" )
    call nc_check( nf90_def_dim(ncIDo, "time", nf90_unlimited, t_dimid), "TroughCutoff", "def dim time TroughCutoff_slope" )
    
    call nc_check( nf90_def_dim(ncIDo, "latitude", ny, y_dimid), "TroughCutoff", "def dim south_north TroughCutoff_slope" )

    call nc_check( nf90_def_dim(ncIDo, "radius", nr, r_dimid), "TroughCutoff", "def dim bottom_top TroughCutoff_slope" )
    
    call nc_check( nf90_def_dim(ncIDo, "longitude", nx, x_dimid), "TroughCutoff", "def dim west_east TroughCutoff_slope" )
    ...
    !Initiailize vars
    call nc_check( nf90_def_var(ncIDo, "HGT_500mb", nf90_real, dimids_nor, varid), "TroughCutoff", "def var hgt TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "m"), "TroughCutoff", "def units hgt TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "geopotential height"), "TroughCutoff", "def units hgt TroughCutoff_slope" )
    
    call nc_check( nf90_def_var(ncIDo, "ASLOPE", nf90_real, dimids, varid), "TroughCutoff", "def var aslope TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "m 100km-1"), "TroughCutoff", "def units aslope TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "average slope function"), "TroughCutoff", "def units aslope TroughCutoff_slope" )
    
    call nc_check( nf90_def_var(ncIDo, "BGSLOPE", nf90_real, dimids, varid), "TroughCutoff", "def var bgslope TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "m 100km-1"), "TroughCutoff", "def units bgslope TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "background slope function"), "TroughCutoff", "def units bgslope TroughCutoff_slope" ) 
    
    call nc_check( nf90_def_var(ncIDo, "So", nf90_real, dimids_nor, varid), "TroughCutoff", "def var So TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "m 100km-1"), "TroughCutoff", "def units So TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "optimal slope function"), "TroughCutoff", "def units So TroughCutoff_slope" )
    
    call nc_check( nf90_def_var(ncIDo, "BGo", nf90_real, dimids_nor, varid), "TroughCutoff", "def var BGo TroughCutoff_slope" )
    ...
    call nc_check( nf90_def_var(ncIDo, "Ro", nf90_real, dimids_nor, varid), "TroughCutoff", "def var Ro TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "m"), "TroughCutoff", "def units Ro TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "optimal radius"), "TroughCutoff", "def units Ro TroughCutoff_slope" )
    
    call nc_check( nf90_def_var(ncIDo, "SR", nf90_real, dimids_nor, varid), "TroughCutoff", "def var Ro TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", ""), "TroughCutoff", "def units Ro TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "slope ratio"), "TroughCutoff", "def units SR TroughCutoff_slope" ) 
    
    call nc_check( nf90_def_var(ncIDo, "ASMASK", nf90_real, dimids_nor, varid), "TroughCutoff", "def var asmask TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", " "), "TroughCutoff", "def units asmask TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "mask for So maxima"), "TroughCutoff", "def units asmask TroughCutoff_slope" ) 
    
    call nc_check( nf90_def_var(ncIDo, "ASMASK2", nf90_real, dimids_nor, varid), "TroughCutoff", "def var asmask2 TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", " "), "TroughCutoff", "def units asmask2 TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "mask for So maxima after distance checks"), "TroughCutoff", "def units asmask2 TroughCutoff_slope" ) 
    
    call nc_check( nf90_def_var(ncIDo, "ASMASK3", nf90_real, dimids_nor, varid), "TroughCutoff", "def var asmask23TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", " "), "TroughCutoff", "def units asmask3 TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "mask for So maxima after shared zmin checks"), "TroughCutoff", "def units asmask3 TroughCutoff_slope" ) 
    
    call nc_check( nf90_def_var(ncIDo, "MASKRo", nf90_real, dimids_nor, varid), "TroughCutoff", "def var MASKRo TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "binary"), "TroughCutoff", "def units MASKRo TroughCutoff_slope" )  
    call nc_check( nf90_put_att(ncIDo, varid, "description ", "binary mask for radius of features after all checks"), "TroughCutoff", "def units MASKRo TroughCutoff_slope" )       
    
    call nc_check( nf90_def_var(ncIDo, "time", nf90_real, t_dimid, varid), "TroughCutoff", "def var time TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "seconds since 1970-01-01 00:00:00.0 0:00"), "TroughCutoff", "def units time TroughCutoff_slope" )  

    call nc_check( nf90_def_var(ncIDo, "latitude", nf90_real, y_dimid, varid), "TroughCutoff", "def var latitude TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "degrees_north"), "TroughCutoff", "def units latitude TroughCutoff_slope" )  
    
    call nc_check( nf90_def_var(ncIDo, "radius", nf90_real, r_dimid, varid), "TroughCutoff", "def var radius TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "m"), "TroughCutoff", "def units radius TroughCutoff_slope" )  
    
    call nc_check( nf90_def_var(ncIDo, "longitude", nf90_real, x_dimid, varid), "TroughCutoff", "def var longitude TroughCutoff_slope" )
    call nc_check( nf90_put_att(ncIDo, varid, "units ", "degrees_east"), "TroughCutoff", "def units longitude TroughCutoff_slope" )  
   
    call nc_check( nf90_enddef(ncIDo), "TroughCutoff", "end define mode TroughCutoff_slope" ) 


    !Write Vars


    call nc_check( nf90_inq_varid(ncIDo, "HGT_500mb", varid), "TroughCutoff", "inq_varid hgt")
    call nc_check( nf90_put_var(ncIDo, varid, hgt, start_nor, ct_nor), "TroughCutoff", "put var hgt TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "ASLOPE", varid), "TroughCutoff", "inq_varid aslope")
    call nc_check( nf90_put_var(ncIDo, varid, aslope, start, ct), "TroughCutoff", "put var aslope TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "BGSLOPE", varid), "TroughCutoff", "inq_varid bgslope")
    call nc_check( nf90_put_var(ncIDo, varid, BG, start, ct), "TroughCutoff", "put var bgslope TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "So", varid), "TroughCutoff", "inq_varid So")
    call nc_check( nf90_put_var(ncIDo, varid, aslopex, start_nor, ct_nor), "TroughCutoff", "put var So TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "BGo", varid), "TroughCutoff", "inq_varid BGo")
    call nc_check( nf90_put_var(ncIDo, varid, BGx, start_nor, ct_nor), "TroughCutoff", "put var BGo TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "BGo-lat", varid), "TroughCutoff", "inq_varid BGo-lat")
    call nc_check( nf90_put_var(ncIDo, varid, BGxlat, start_nor, ct_nor), "TroughCutoff", "put var BGo-lat TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "BGo-lon", varid), "TroughCutoff", "inq_varid BGo-lon")
    call nc_check( nf90_put_var(ncIDo, varid, BGxlon, start_nor, ct_nor), "TroughCutoff", "put var BGo-lon TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "Ro", varid), "TroughCutoff", "inq_varid Ro")
    call nc_check( nf90_put_var(ncIDo, varid, aslopexr, start_nor, ct_nor), "TroughCutoff", "put var Ro TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "SR", varid), "TroughCutoff", "inq_varid SR")
    call nc_check( nf90_put_var(ncIDo, varid, srat, start_nor, ct_nor), "TroughCutoff", "put var SR TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "ASMASK", varid), "TroughCutoff", "inq_varid asmask")
    call nc_check( nf90_put_var(ncIDo, varid, asmask, start_nor, ct_nor), "TroughCutoff", "put var asmask TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "ASMASK2", varid), "TroughCutoff", "inq_varid asmask2")
    call nc_check( nf90_put_var(ncIDo, varid, asmask2, start_nor, ct_nor), "TroughCutoff", "put var asmask2 TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "ASMASK3", varid), "TroughCutoff", "inq_varid asmask3")
    call nc_check( nf90_put_var(ncIDo, varid, asmask3, start_nor, ct_nor), "TroughCutoff", "put var asmask3 TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "MASKRo", varid), "TroughCutoff", "inq_varid MASKRo")
    call nc_check( nf90_put_var(ncIDo, varid, gridmask_all, start_nor, ct_nor), "TroughCutoff", "put var MASKRo TroughCutoff_slope" )
    
    call nc_check( nf90_inq_varid(ncIDo, "time", varid), "TroughCutoff", "inq_varid lat")
    call nc_check( nf90_put_var(ncIDo, varid, time, start_t, ct_t), "TroughCutoff", "put var time TroughCutoff_slope" )
    call nc_check( nf90_inq_varid(ncIDo, "latitude", varid), "TroughCutoff", "inq_varid lat")
    call nc_check( nf90_put_var(ncIDo, varid, lat, start_y, ct_y), "TroughCutoff", "put var lat TroughCutoff_slope" )
    call nc_check( nf90_inq_varid(ncIDo, "radius", varid), "TroughCutoff", "inq_varid radius")
    call nc_check( nf90_put_var(ncIDo, varid, radius, start_r, ct_r), "TroughCutoff", "put var radius TroughCutoff_slope" )
    call nc_check( nf90_inq_varid(ncIDo, "longitude", varid), "TroughCutoff", "inq_varid lon")
    call nc_check( nf90_put_var(ncIDo, varid, lon, start_x, ct_x), "TroughCutoff", "put var lon TroughCutoff_slope" )
    
    ! Close the output file
    call nc_check( nf90_close(ncIDo), "TroughCutoff", "close " // trim(doutfile) )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! 9-point smoother, adapted from NCL https://www.ncl.ucar.edu/Document/Functions/Built-in/smth9.shtml

subroutine smth9(var,dimx,dimy,ew,cw,npasses) 
implicit none
          integer,intent(in)	:: dimx,dimy,npasses
	  real,intent(in) 	:: ew,cw			! EdgeWeight and CornerWeight
          real			:: var(dimx,dimy)
	  real			:: prev(dimx,dimy)
          integer		:: xind,yind,sind

prev = var
do sind=1,npasses
  do yind=2,dimy-1								! Values at 90 and -90 are constant
    do xind=2,dimx-1								! Weighted avereage of CENTER(W=(1-ew-cw)), UP/RIGHT/DOWN/LEFT(W=ew), and UL/UR/LR/LL(W=cw)
      var(xind,yind) = ((1.0-ew-cw)*prev(xind,yind))+((ew/4.0)*(prev(xind,yind+1)+prev(xind+1,yind)+prev(xind,yind-1)+prev(xind-1,yind)))+((cw/4.0)*(prev(xind-1,yind+1)+prev(xind+1,yind+1)+prev(xind+1,yind-1)+prev(xind-1,yind-1)))
    end do
    var(1,yind)  = ((1.0-ew-cw)*prev(1,yind))+((ew/4.0)*(prev(1,yind+1)+prev(2,yind)+prev(1,yind-1)+prev(dimx,yind)))+((cw/4.0)*(prev(dimx,yind+1)+prev(2,yind+1)+prev(2,yind-1)+prev(dimx,yind-1)))
    var(dimx,yind) = ((1.0-ew-cw)*prev(dimx,yind))+((ew/4.0)*(prev(dimx,yind+1)+prev(1,yind)+prev(dimx,yind-1)+prev(dimx-1,yind)))+((cw/4.0)*(prev(dimx-1,yind+1)+prev(1,yind+1)+prev(1,yind-1)+prev(dimx-1,yind-1)))
  end do
  prev   = var
end do 
