...
real, parameter			:: SMP = 0.50		!Smoothing parameter p from NCL smth9
real, parameter			:: SMQ = 0.25		!Smoothing parameter q from NCL smth9
integer, parameter		:: NSM = 3		!Number of passes to smooth over
real				:: maxr, minr, srmax
...
real				:: d, haversine, hgtmin, c1c2_overlap, o
...
character(len=10)		:: smooth		!smoothing option - 9 point (smth9), none (none)
...
outdir = "/glade/work/klupo/postdoc/kasugaEA21/version7/" // trim(varstr) // "/"
outfile = trim(outdir) // infile(32:55) // ".dat"

if(debug.eq."debug".or.debug.eq."debugonly")then
  doutfile = trim(outdir) // "gridded/" // infile(32:55) // "." // "debug" // ".nc"
endif
if(debug.eq."binary")then
  doutfile = trim(outdir) // "gridded/" // infile(32:55) // "." // "binary" // ".nc"
endif
...
do r = 1, nr
  ...
  do y=1, ny  									
    if (abs(lat(y)).lt.19.75.or.abs(lat(y)).gt.70.25) cycle		! Don't compute values poleward of 70 degrees or between 20S and 20N
...
do a = 1, np1
  ...
  if(features_p1(a,1).lt.5.0) then					! Fail test - So is less that or equal to zero (or a larger user defined threshold value)
    features_p1(a,10) = 0
    cycle
  end if
...
! Check for shared height minima
do a=1, np3
...
! Check for engulfed troughs within larger troughs
do a=1, np3
  ...
  do i=1, np3
    ...
    if(o.gt.0.80) then
       features_p3(y,12) = 0						! Fail feature (a or i) if it has the smaller Ro of two 80% overlapping features
    end if
  end do
end do
...
!!!!Write to text file!!!!
  ...
  fstr = "(A7, 1x, A7, 1x, A11, 1x, A7, 1x, A7, 1x, A7, 1x, A4, 1x, A15, 1x, A16, 1x, A16, 1x, A7, 1x, A7, 1x, A7, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11, 1x, A11)"
  write(UNUM,fstr) "ITIME","FHOUR","So(m/100km)","LAT(N)","LON(E)","Ro(km)","SR","BGo(m/100km)","BGo-lat(m/100km)","BGo-lon(m/100km)","ZMIN(m)","ZLAT(N)","ZLON(E)","Z850(m)","Z500(m)","Z200(m)","T850(K)","T500(K)","T200(K)","U850(m/s)","U500(m/s)","U200(m/s)","V850(m/s)","V500(m/s)","V200(m/s)","MR850(g/kg)","MR500(g/kg)","MR200(g/kg)"

  fstr = "(A11, 1x, A4, 1x, F6.2, 1x, F6.2, 1x, F6.2, 1x, F10.2, 1x, F6.2, 1x, F6.2, 1x, F6.2, 1x, F6.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F8.2, 1x, F5.1,1x,F5.1,1x,F5.1,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.2,1x,F7.4,1x,F7.4,1x,F7.4)"
  do b=1,np  
    write(UNUM,fstr) itime, fhour, features(b,1), features(b,2), features(b,3), features(b,4)/1000.0, features(b,5), features(b,6), features(b,7), features(b,8), features(b,9), features(b,10), features(b,11), features(b,12), features(b,13), features(b,14), features(b,15), features(b,16), features(b,17), features(b,18), features(b,19), features(b,20), features(b,21), features(b,22), features(b,23), features(b,24), features(b,25), features(b,26)
  end do 
  close(UNUM)
  call cpu_time(FN)
